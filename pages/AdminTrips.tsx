...
export const AdminTrips: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // ✅ IMPORTANT: Firestore NO accepta undefined.
    // Per això, només afegim "price" i "notes" si tenen valor.
    const payload: any = {
      title: title.trim(),
      date: date.trim(),
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="rounded-3xl border bg-white/70 backdrop-blur shadow-sm p-10 text-center">
          ...
          <p className="text-slate-600 mt-2">
            No tens permisos per gestionar sortides.
          </p>
        </div>
      </div>
    );
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <p className="text-slate-600 mt-1">
                Crear, publicar/ocultar, cancel·lar i esborrar (només admin).
              </p>
            </div>
          ...
          {list.length === 0 ? (
            ...
            list.map((t) => (
              <TripCard
                ...
                onCancel={() =>
                  cancelTrip(t.id, prompt("Motiu (opcional):") || "")
                }
        ...
        <div className="min-w-0">
          ...
          {isCancelled && (trip as any).cancelledReason ? (
            <div className="mt-2 text-sm text-red-700">
              ...
              {(trip as any).cancelledReason}
            </div>
          ) : null}
        </div>

        {/* ✅ Accions: només ADMIN (així evitem errors de rules) */}
          ...
          {isAdmin && (
            <button
              onClick={() => {
                if (
                  confirm(
                    "Segur que vols ESBORRAR definitivament esta sortida?"
                  )
                )
                  onDelete();
              }}
